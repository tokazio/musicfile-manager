...
public class LibraryDAO {
	...
	public Library read(final String filePath) {
		...
		if (xmlFile.exists()) {
			if (xmlFile.canRead()) {
				...
				LOGGER.error(new StringBuffer("The given file [").append(filePath).append("] is not readable")
						.toString());
			}
		} else {
	 ...
	 * @return true if the given library is successfully persisted, false otherwise
	 */
	public boolean write(Library lib, final String filePath) {
		boolean result = false;
		  XMLOutputter outputter = new XMLOutputter(Format.getPrettyFormat()); //TODO set configurable
		    try {
		      outputter.output(lib.getRootElement(), System.out);
		      result = true;
		    }
		    catch (IOException e) {
		    	LOGGER.error(new StringBuffer("Unable to persist the library in the file [").append(filePath).append("]:\n")
